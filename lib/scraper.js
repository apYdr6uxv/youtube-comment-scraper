            ...
            return puppeteer.launch(
                ...
                return browser.newPage().then((page) => {
                    ...
                    return page.goto(url).then((response) => {
                        ...
                        if(!response.ok()) {
                            return Promise.reject(`Open url returns ${response.status()}`);
                        }
                        ...
                        return new Promise((resolve, reject) => {
                            ...
                            // If not, wait a bit and try again, until
                            // loading is finished.
                            console.log("Checking header is loaded...")
                            function check_header() {
                                console.log(
                                    "Checking header information has been loaded..."
                                    + " [Attempt "
                                    + (1+retries)
                                    + "/"
                                    + MAX_RETRIES
                                ...
                                page.evaluate(function() {
                                    return document.getElementsByTagName("ytd-item-section-renderer").length;
                                }).then((res) => {
                                    retries++;
                                    if (res != 0) {
                                        ...
                                        if(retries < MAX_RETRIES){
                                            ...
                                            console.log("Waiting a bit and try again")
                                            setTimeout(check_header, RETRY_INTERVAL_MS);
                                        }
                                        else {
                                            let error_message =
                                                "Waiting for the header information to load, timed out after "
                                                + MAX_RETRIES
                                                + "×"
                                                + RETRY_INTERVAL_MS
                                                + "ms."
                        ...
                        return page.evaluate(function() {
                            ...
                            // load-more-button and wait a bit,
                        ...
                        const user = $("ytd-video-owner-renderer ytd-channel-name a");
                        resolve({
                            id: id,
                            channel: {
